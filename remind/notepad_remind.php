...
$query1="select * from notepad where DATE_FORMAT(time,'%m%d') >= DATE_FORMAT(now(),'%m%d') and DATE_FORMAT(time,'%m%d') <= DATE_FORMAT(DATE_ADD(now(), Interval +10 day),'%m%d');";
...
{
	$query2="select id from customer_notepad where notepad_number=$row1[0];";
